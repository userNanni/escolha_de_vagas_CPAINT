...
function App() {
  ...
  const fetchVagasStatus = useCallback(async (): Promise<Escolha[]> => {
    ...
    return (
      (data as VagasStatusRaw[])?.map((item) => ({
        id: item.om,
        OM: item.om,
        state: item.estado,
  ...
  const fetchVisibleCard = useCallback(async (): Promise<{
    cardData: Pessoa | null;
    lastState?: string;
  }> => {
    const { data, error } = await supabase
      .from(TABLES.PESSOAS)
  ...
  const handlePessoasRealtimeChange = useCallback(
    async (payload: any) => {
      ...
      safeSetState((prev) => {
        ...
        switch (payload.eventType) {
          case "INSERT":
            ...
            break;
          case "UPDATE":
            updatedPessoas = updatedPessoas.map((p) =>
              p.id === newPessoa.id ? newPessoa : p
            );
            break;
          case "DELETE":
        ...
        if (newPessoa) {
          ...
          if (updatedVisibleCard && newPessoa.id === updatedVisibleCard.id) {
            updatedVisibleCard =
              newPessoa.show_card && !newPessoa.hide_card ? newPessoa : null;
  ...
  return (
    <div className="grid h-screen w-screen grid-cols-2 items-center justify-center gap-8 p-8 bg-slate-950">
      <div className="justify-self-center">{brazilComponent}</div>
      ...
      {state.visibleCardData && <PessoaCard cardData={state.visibleCardData} />}
    </div>
  );
}
